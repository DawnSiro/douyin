...
func main() {
	...
	addr, err := net.ResolveTCPAddr(constant.TCP, constant.FeedServiceAddr)
	if err != nil {
		panic(err)
	}
	Init()
	provider.NewOpenTelemetryProvider(
		provider.WithServiceName(constant.FeedServiceName),
	...
	svr := feedservice.NewServer(new(FeedServiceImpl),
		...
		server.WithServerBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: constant.FeedServiceName}),
	)
	err = svr.Run()
	if err != nil {
		klog.Fatal(err)
