...
func IsFollow(ctx context.Context, userID uint64, toUserID uint64) bool {
	// 未登录默认未关注
	...
	// 查不到关注记录则为未关注
	result := global.DB.WithContext(ctx).Where("user_id = ? AND to_user_id = ? AND is_deleted = ?",
...
func IsFriend(ctx context.Context, userID uint64, toUserID uint64) bool {
	// 默认不能给自己发消息
	...
	result := global.DB.WithContext(ctx).Where("(user_id = ? AND to_user_id = ? OR user_id = ? AND to_user_id = ?) AND is_deleted = ?",
...
func Follow(ctx context.Context, userID uint64, toUserID uint64) error {
	if userID == 0 || toUserID == 0 {
...
func CancelFollow(ctx context.Context, userID uint64, toUserID uint64) error {
	if userID == 0 || toUserID == 0 {
		return errno.UserRequestParameterError
	}
	...
	return global.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		// 去除关注的关系
		return global.DB.WithContext(ctx).Model(relation).Where("is_deleted = ?", constant.DataNotDeleted).
			Update("is_deleted", constant.DataDeleted).Error
	})

}

func GetFollowList(ctx context.Context, userID uint64) ([]*User, error) {
	relations := make([]*Relation, 0)
	res := make([]*User, 0)

	result := global.DB.WithContext(ctx).Where("user_id = ? AND is_deleted = ?", userID, constant.DataNotDeleted).Find(&relations)
	if result.RowsAffected == 0 {
		return res, nil
	}

	for i := 0; i < len(relations); i++ {
		u, err := SelectUserByID(ctx, relations[i].ToUserID)
		if err != nil {
			return nil, err
		}
...
func GetFollowerList(ctx context.Context, userID uint64) ([]*User, error) {
	relations := make([]*Relation, 0)
	res := make([]*User, 0)

	result := global.DB.WithContext(ctx).Where("to_user_id = ? AND is_deleted = ?", userID, constant.DataNotDeleted).Find(&relations)
	if result.RowsAffected == 0 {
		return res, nil
	}

	for i := 0; i < len(relations); i++ {
		u, err := SelectUserByID(ctx, relations[i].UserID)
		if err != nil {
			return nil, err
		}
		res = append(res, u)
...
func GetFriendList(ctx context.Context, userID uint64) ([]*User, error) {
	friendUserID := make([]int64, 0)
	res := make([]*User, 0)

	// 查询关注自己的所有粉丝的 userID
	followerQuery := global.DB.WithContext(ctx).Select("user_id").Table(constant.RelationTableName).
		Where("to_user_id = ? AND is_deleted = ?", userID, constant.DataNotDeleted)
	// 查询自己关注的，并且在自己的粉丝 userID 集合里的用户
	err := global.DB.WithContext(ctx).Select("to_user_id").Table(constant.RelationTableName).
	...
	err = global.DB.WithContext(ctx).Where("id IN (?)", friendUserID).Find(&res).Error
	if err != nil {
		return nil, err
	}
	return res, nil
...
func SelectFollowDataListByUserID(ctx context.Context, userID uint64) ([]*RelationUserData, error) {
	res := make([]*RelationUserData, 0)
	err := global.DB.WithContext(ctx).Select("u.id AS uid, u.username, u.following_count, u.follower_count, u.avatar,"+
...
func SelectFollowerDataListByUserID(ctx context.Context, userID uint64) ([]*RelationUserData, error) {
	res := make([]*RelationUserData, 0)
	err := global.DB.WithContext(ctx).Select("u.id AS uid, u.username, u.following_count, u.follower_count, u.avatar,"+
		"IF(r.is_deleted = ?, TRUE, FALSE) AS is_follow", constant.DataNotDeleted).Table("`user` AS u").
