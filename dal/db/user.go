...
func CreateUser(ctx context.Context, user *User) (userID uint64, err error) {
	// 这里不指明更新的字段的话，会全部赋零值，就无法使用数据库的默认值了
	if err := global.DB.WithContext(ctx).Select("username", "password").Create(user).Error; err != nil {
		return 0, err
	}
	return user.ID, nil
}

func SelectUserByID(ctx context.Context, userID uint64) (*User, error) {
	res := User{ID: userID}
	if err := global.DB.WithContext(ctx).First(&res).Error; err != nil {
		return nil, err
	}
	return &res, nil
}
...
func SelectUserByName(ctx context.Context, username string) ([]*User, error) {
	res := make([]*User, 0)
	if err := global.DB.WithContext(ctx).Where("username = ?", username).Limit(1).Find(&res).Error; err != nil {
		return nil, err
	}
	return res, nil
}

func IncreaseUserFavoriteCount(ctx context.Context, userID uint64) (int64, error) {
	user := &User{
		ID: userID,
	}
	err := global.DB.WithContext(ctx).First(&user).Error
	if err != nil {
		return 0, err
	}
	if err := global.DB.WithContext(ctx).Model(&user).Update("favorite_count", user.FavoriteCount+1).Error; err != nil {
		return 0, err
	}
	return user.FavoriteCount, nil
}

func DecreaseUserFavoriteCount(ctx context.Context, userID uint64) (int64, error) {
	user := &User{
		ID: userID,
	}
	err := global.DB.WithContext(ctx).First(&user).Error
